...
async def begin(message: types.Message):
    ...
    if language == 'uz':
        await message.answer(
            "🙂 Bizga bo'lgan biror fikringiz yoki taklifingiz bo'lsa yozib qoldiring. Sizning fikringiz biz uchun "
            "muhim!",
            reply_markup=cancel(language))
        await Comment.text.set()
    elif language == 'en':
        await message.answer(
            "🙂 If you have any comments or suggestions for us, write them down. Your opinion is important to us!",
            reply_markup=cancel(language))
        await Comment.text.set()
    else:
        await message.answer(
            "🙂 Если у вас есть какие-либо комментарии или предложения к нам, запишите их. Ваше мнение важно для нас!",
            reply_markup=cancel(language))
        await Comment.text.set()
    ...
    if message.text in ["❌ Отменить", "❌ Bekor qilish", "❌ Cancel"]:
        if language == 'uz':
            ...
            await message.answer("✅ Welcome to the main menu\n" 
                                 ...
                                 f"magazines! Are you starting to order?",
